...
<head>
    ...
    <style>
        .gps-indicator { width: 12px; height: 12px; border-radius: 50%; animation: pulse 2s infinite; display: inline-block; margin-right: 8px; }
        .gps-excellent { background: #22c55e; }
        .gps-good { background: #3b82f6; }
        .gps-poor { background: #ef4444; }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.6; } }
        .location-info { background: #f1f5f9; border-radius: 6px; padding: 0.75rem; margin-top: 0.5rem; }
        .progress-bar-container { background: #e5e7eb; border-radius: 8px; overflow: hidden; height: 8px; }
        .progress-bar-fill { background: linear-gradient(90deg, #3b82f6, #22c55e); height: 100%; transition: width 0.3s ease; }
        .offline-banner { position: fixed; top: 0; left: 0; right: 0; background: #dc3545; color: white; padding: 10px; text-align: center; z-index: 9999; display: none; }
        .offline-banner.show { display: block; }
        .pending-banner { position: fixed; bottom: 20px; right: 20px; background: #ffc107; color: #000; padding: 15px; border-radius: 8px; z-index: 9999; display: none; max-width: 300px; }
        .pending-banner.show { display: block; }
        .form-footer { margin-top: 3rem; padding: 2rem 0; border-top: 1px solid #dee2e6; text-align: center; background: #f8f9fa; }
    </style>
</head>
<body class="bg-light">
    <div class="offline-banner" id="offlineBanner">
        <i class="fas fa-wifi-slash"></i> Working Offline - Data will sync when connected
    </div>

    <div class="pending-banner" id="pendingBanner">
        <div><strong><span id="pendingCount">0</span> responses waiting to sync</strong></div>
        <button class="btn btn-sm btn-dark mt-2" onclick="syncNow()">Sync Now</button>
        <button class="btn btn-sm btn-outline-dark mt-2 ms-2" onclick="clearPending()">Clear</button>
    </div>
    ...
    <div class="form-footer">
        <div class="container">
            <p class="text-muted mb-2"><?php echo $lang === 'es' ? '¿Necesitas crear tus propios formularios?' : 'Need to create your own forms?'; ?></p>
    ...
    <script>
        ...
        let pendingResponses = JSON.parse(localStorage.getItem('pendingResponses') || '[]');
        ...
        const formConfig = {
            ...
            formCode: <?php echo json_encode($form_code); ?>
        };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (formConfig.hasForm) {
                ...
                if (formConfig.responses && formConfig.responses.length > 0) {
                    showResponse(formConfig.responses.length - 1);
                }
            }
            ...
            window.addEventListener('online', function() {
                isOnline = true;
                checkOnlineStatus();
                setTimeout(syncPendingResponses, 2000);
            });
        ...
        function showPendingBanner() {
            ...
            if (pendingResponses.length > 0) {
                count.textContent = pendingResponses.length;
                banner.classList.add('show');
            } else {
                banner.classList.remove('show');
            }
        ...
        function saveOfflineResponse() {
            ...
            for (let [key, value] of formData.entries()) {
                if (key.endsWith('[]')) {
                    ...
                    if (!responseData[baseKey]) {
                        responseData[baseKey] = [];
                    }
                    responseData[baseKey].push(value);
                } else {
                    responseData[key] = value;
                }
            }
            
            responseData.uniqueId = generateUniqueId();
        ...
        function generateUniqueId() {
            ...
            return `${formConfig.formCode}_${timestamp}_${random}`.replace(/[^a-zA-Z0-9_]/g, '');
        }

        function showOfflineSuccess() {
            const alertHtml = `
                <div class="alert alert-info alert-dismissible fade show">
                    <i class="fas fa-cloud-upload-alt"></i> Response saved offline. Will sync when connection is restored.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            `;
        ...
        function syncPendingResponses() {
            if (!isOnline || pendingResponses.length === 0) return;
            
            console.log('Syncing', pendingResponses.length, 'pending responses');
            
            const syncButton = document.querySelector('#pendingBanner button');
            if (syncButton) {
            ...
            let syncedCount = 0;
            let totalToSync = pendingResponses.length;
            
            const responsesToSync = [...pendingResponses];
            
            responsesToSync.forEach((responseData, index) => {
                setTimeout(() => {
                    syncSingleResponse(responseData, () => {
                        syncedCount++;
                        console.log(`Synced ${syncedCount}/${totalToSync}`);
        ...
        function syncSingleResponse(responseData, callback) {
            const formData = new FormData();
            
            Object.keys(responseData).forEach(key => {
                if (key === 'uniqueId' || key === 'timestamp') return;
            ...
            .then(data => {
                if (data.includes('response_saved') || data.includes('exitosamente')) {
                    console.log('Response synced successfully');
                    if (callback) callback();
                } else {
                    console.log('Sync failed for response');
                    if (callback) callback();
                }
            })
            .catch(error => {
                console.log('Error syncing response:', error);
                if (callback) callback();
            });
        }
        ...
        function showSyncComplete() {
            const alertHtml = `
                <div class="alert alert-success alert-dismissible fade show">
                    <i class="fas fa-check-circle"></i> All responses synced successfully!
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            `;
        ...
        function removePendingResponse(index) {
        ...
        function syncNow() {
            if (isOnline && pendingResponses.length > 0) {
                ...
                alert('No pending responses to sync');
            }
        }

        function startLocationTracking() {
            updateGPSStatus('getting', formConfig.strings.getting_location || 'Getting location...');
        ...
        function enableSubmitButton() {
            ...
            if (submitBtn && submitText && currentLocation) {
                ...
                const baseText = formConfig.strings.submit_response || 'Submit Response';
                submitText.textContent = isOnline ? baseText : baseText + ' (Offline)';
            }
        }

        function showResponse(index) {
            if (!formConfig.responses || formConfig.responses.length === 0) return;
            
            currentResponseIndex = Math.max(0, Math.min(index, formConfig.responses.length - 1));
            const response = formConfig.responses[currentResponseIndex];
            ...
            if (formConfig.fieldsConfig && formConfig.fieldsConfig.length > 0) {
        ...
        function updateNavigationButtons() {
            ...
            if (responseInfo) {
                const displayIndex = formConfig.responses.length - currentResponseIndex;
                responseInfo.textContent = `${displayIndex} / ${formConfig.responses.length}`;
            }
            
            if (prevBtn) {
                prevBtn.disabled = currentResponseIndex >= formConfig.responses.length - 1;
            }
        ...
        function prevResponse() {
            if (currentResponseIndex < formConfig.responses.length - 1) {
        ...
        function changeForm() {
            window.location.href = window.location.pathname + '?lang=' + (formConfig.strings.language || 'en');
        }
